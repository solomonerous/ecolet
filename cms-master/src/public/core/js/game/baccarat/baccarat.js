...
function setDataCurrentBet(betList) {
    ...
    betList.forEach(betData => {
        if (betData.bet.player > 0) betDoorList.PLAYER.push({
            username: betData.username,
            amount: betData.bet.player,
            time: betData.bet_time
        });
        if (betData.bet.player_double > 0) betDoorList.PLAYER_DOUBLE.push({
            username: betData.username,
            amount: betData.bet.player_double,
            time: betData.bet_time
        });
        if (betData.bet.banker > 0) betDoorList.BANKER.push({
            username: betData.username,
            amount: betData.bet.banker,
            time: betData.bet_time
        });
        if (betData.bet.banker_double > 0) betDoorList.BANKER_DOUBLE.push({
            username: betData.username,
            amount: betData.bet.banker_double,
            time: betData.bet_time
        });
        if (betData.bet.draw > 0) betDoorList.DRAW.push({
            username: betData.username,
            amount: betData.bet.draw,
            time: betData.bet_time
        });
        if (betData.bet.super > 0) betDoorList.SUPER.push({
            username: betData.username,
            amount: betData.bet.super,
            time: betData.bet_time
        });
    });
