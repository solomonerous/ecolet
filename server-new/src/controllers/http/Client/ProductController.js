...
module.exports = {
    ...
    category: async (req, res) => {
        try {
            ...
            if (!subnamesConfig.includes(productType.toUpperCase())) return res.status(200).json({
                status: false,
                msg: "Nhà cung cấp trò chơi này hiện tại không khả dụng! Xin trân thành xin lỗi quý khách hàng vì sự thiếu sót này."
            });
            if (!gameConfig.hasOwnProperty(productType.toUpperCase())) return res.status(200).json({
                status: false,
                msg: "Nhà cung cấp trò chơi này hiện tại không khả dụng! Xin trân thành xin lỗi quý khách hàng vì sự thiếu sót này."
            });
            if (!gameTypeConfig.includes(gameType.toUpperCase())) return res.status(200).json({
                status: false,
                msg: "Nhà cung cấp trò chơi này hiện tại không khả dụng! Xin trân thành xin lỗi quý khách hàng vì sự thiếu sót này."
            });
