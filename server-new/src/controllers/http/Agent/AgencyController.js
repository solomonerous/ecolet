...
module.exports = {
    ...
    listAgency: async (req, res) => {
        try {
            ...
            if (!!page && !!kmess) {
                ...
                const total = await AgencyRefModel.scope(assocScopes, "withAgencyInfo").count({where: match});

                let getUsers = await AgencyRefModel.scope(assocScopes, "withAgencyInfo").findAll({
                    where: match,
    ...
    AgencyInfo: async (req, res) => {
        try {
            ...
            //withAgencyInfo
            const user = await UserModel.scope({method: ['withAgencyInfo']}, "withAgencyInfo").findOne({
    ...
    calculatorProfit: async (req, res) => {
        try {
            ...
            const totalUsers = await AgencyRefModel.findAll({
                ...
                include: [
                    {
                        model: UserModel.scope('withRoleUser', "withAgencyInfo"),
                        as: "userInfo",
                        attributes: {exclude: ["password", "id", "deletedAt", "code", "status", "role", "updatedAt"]},
                        require: false
                    }
            ...
            for (const userId of listUserRef) taskPromise.push(UserModel.findOne({
                where: {id: userId},
