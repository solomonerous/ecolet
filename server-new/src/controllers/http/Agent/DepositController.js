    ...
    listDeposit: async (req, res) => {
        try {
            ...
            if (!!page && !!kmess) {
                ...
                const total = await BankHistoryModel.scope(assocScopes, "withAgencyInfo").count({where: match});
                const getData = await BankHistoryModel.scope(assocScopes, "withAgencyInfo").findAll({
                    where: match,
