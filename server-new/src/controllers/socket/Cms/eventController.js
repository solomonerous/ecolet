...
const AuthController = require('./EvenHandles/Authentication');
const Message = require('./EvenHandles/Message');
...
const EventSocket = {
    initSocketParameter: (socket) => {
        ...
        socket.red = (data) => {
            try {
                socket.readyState == 1 && socket.send(JSON.stringify(data));
            } catch (err) {
            }
        };
    },
            ...
            if (!!message) {
                ...
                if (socket.auth == false && !!message.authentication) {
                    ...
                    if (!!auth.status) {
                        ...
                        // add to list online 
                        SocketSetup.addToListOnline(socket, socketMain.admins);
    ...
    onClose: async (data, socket, socketMain) => {
        socket.auth = false;
        if (socket.UID !== null) {
            SocketSetup.removeFromListOnline(socket, socketMain.admins);
