        ...
        if (!!data && !!data.code) {
            ...
            if (arrUserId.length > 0) {
                ...
                for (const user of getUserLists) {
                    ...
                    let totalNapBank = 0;
                    for (const rows of findHistoryBank) totalNapBank += Number(rows.money);
                    let totalNapThe = 0;
                    for (const rows of findHistoryThecao) totalNapThe += Number(rows.nhan);
                    let totalRut = 0;
                    for (const rows of findHistoryRut) totalRut += Number(rows.money);
                    ...
                    let getBonusReward = await UserHistory.find(
                        ...
                        , 'red').exec();
                    let bonusReward = 0;
                    for (const rows of getBonusReward) bonusReward += Number(rows.red);
